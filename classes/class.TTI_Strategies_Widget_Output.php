 ...
 */
class TTI_Strategies_Widget_Output {
	...
	function widget_output() {
		...
		global $wp_query, $TTI_Strategies_CPT;
		...
		$wp_query = new WP_Query( array( 'post__in' => array( $this->_instance['strategy_id'] ), 'post_type' => $TTI_Strategies_CPT->post_type ) );
		...
		if ( $this->_instance['show_image'] && $image ) {
			...
			$banner = ( $this->_instance['show_featured_text'] && $text = tti_strategies_get_strategy_meta( 'featured_text' ) ) ? sprintf( '<div class="strategy-featured-text-banner">%s</div>', $text ) : '';

			printf( '<div class="tti-strategy-featured-image image-%s"><a class="%s" href="%s" title="%s">%s %s</a></div>', esc_attr( $this->_instance['image_alignment'] ), esc_attr( $this->_instance['image_alignment'] ), get_permalink(), the_title_attribute( 'echo=0' ), $image, $banner );
		...
		if( ! empty( $this->_instance['show_title'] ) || ! empty( $this->_instance['show_objective'] ) ){
			...
			if ( ! empty( $this->_instance['show_title'] ) ) {
				...
				$title = get_the_title() ? get_the_title() : __( '(no title)', 'd3js-circle-packing' );
				 ...
				 * }
				 */
				$title = apply_filters( 'tti_strategies_featured_strategy_title', $title, $this->_instance, $this->_args );
			...
			$this->_instance['show_objective'] ? tti_strategies_do_by_line() : '';
		...
		if ( ! empty( $this->_instance['show_content'] ) || ! empty( $this->_instance['show_price'] ) || ! empty( $this->_instance['more_text'] ) ) {
			...
			echo empty( $this->_instance['show_price'] ) ? '' : sprintf( '<p>%s</p>', tti_strategies_get_price() );
