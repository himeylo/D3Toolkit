var cost = d3.select('.cost-container').append('svg')
...
cost.append('svg:image')
  .attr('xlink:href', '../../wp-content/plugins/d3js-circle-packing/framework/images/dollar-sign.svg')
  .attr('x', 0)
  .attr('y', '80%')
  .attr('width', '17%')
  .attr('height', '20%');
...
var time = d3.select('.time-container').append('svg')
...
var impact = impactCont.append('svg')
  .attr('class', 'impact three-fourths');

if (!isNaN(impact.style('width').replace('px', '') / 2 * 0.90)) {
  impact.attr('height', impact.style('width').replace('px', '') / 2 * 0.90);
}

// state
impact.append('circle')
...
solutionDiv.addEventListener('mouseover', function(e) {
  if (typeof e.target.classList[1] === 'undefined') { return false; }
  var delay = setTimeout(function() {
    var id = e.target.classList[1];
    var strat = jQuery('.solution.' + id);

    // Set topic text.
    d3.select('.topic-text').text(strategy.subcategory(strat.data('subcategory'))).classed('hidden', false);
    d3.select('.topic-angle').classed('hidden', false);

    // Set title.
    d3.select('.chart-title').text(strat.data('title')).classed('hidden', false);

    var chartIcon = typeof strat.data('category') !== 'undefined' ? strat.data('category').join(' ') : '';
    // Set icon class.
    ...
    // Set image.
    d3.select('.chart-image').attr('src', strat.data('chart-img')).classed('hidden', false);
    if (strat.data('desc') !== '') {
    ...
    for (var i = 1; i <= 5; i++) {
      ...
      if (i <= strategy.costNumSingle(strat.data('cost'))) {
        d3Cost.transition().style('fill', strategy.color(strat.data('category')));
      } else {
    ...
    for (i = 0; i < impactCount; i++) {
      d3.select('.' + impacts[i].toLowerCase()).transition().style('fill', strategy.color(strat.data('category')));
    }
    ...
    for (i = 0; i < whoCount; i++) {
      if (who[i] !== 'none') {
        var divW = whoCont.append('div')
          .attr('class', 'who-icon');
        divW.append('img')
          .attr('src', '../../wp-content/plugins/d3js-circle-packing/framework/who-hurdle-icons/' + who[i] + '.svg')
    ...
    for (i = 0; i < hurdleCount; i++) {
      if (hurdle[i] !== 'none') {
        var divH = hurdleCont.append('div')
          .attr('class', 'hurdle-icon');
        divH.append('img')
          .attr('src', '../../wp-content/plugins/d3js-circle-packing/framework/who-hurdle-icons/' + hurdle[i] + '.svg')
          .attr('class', 'who-hurdle-icon');
        divH.append('p')
          .attr('class', strat.data('category'))
  ...
  this.onmouseout = function() {
    ...
    d3.select('.time-value').transition().style('fill', '#c1c1c1').attr('width', '100%');
...
function offset(el) {
  ...
  return { elTop: rect.top + scrollTop, elLeft: rect.left + scrollLeft, top: scrollTop, left: scrollLeft };
}
