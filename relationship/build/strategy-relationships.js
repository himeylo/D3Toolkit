...
if ((folder === 'carteeh') | (folder === 'carteeh3')) {
  // base = "/wp-content/plugins/d3-toolkit/relationship/json/";
  // baseLinks = "/wp-content/plugins/d3-toolkit/";
} else {
  base = '/wp-content/plugins/d3-toolkit/relationship/json/';
  baseLinks = '/wp-content/plugins/d3-toolkit/framework/';
  // base = "/wp-json/wp/v2/";
...
function circles(strats) {
  ...
  var circle = svg
    ...
    .attr('class', function (d) {
      return d.parent
        ? d.children
          ? 'node'
          : 'node node-leaf parent-' + d.parent.id
        : 'node node-root';
    })
    ...
    .on('mouseover', function (d) {
      if (
        ...
        typeof d.parent.id !== 'undefined'
      ) {
        var classes =
          'label depth' + d.depth + ' id-' + d.id + ' parent-' + d.parent.id;
        if (d.depth > 1) {
          classes += ' allow-pointer-events';
        }
        d3.select('foreignObject.id-' + d.id + '.parent-' + d.parent.id).attr(
          'class',
          classes + ' hover-text'
        );
      }
  ...
  svg
    ...
    .style('fill-opacity', function (d) {
      return d.parent === strats ? 1 : 0;
    })
    .style('display', function (d) {
      return d.parent === strats ? 'inline' : 'none';
    })
    .append('xhtml:body')
    .html(function (d) {
  ...
  d3.selectAll('.node-leaf').on('click', function (d) {
    if (focus !== d.parent) {
      zoom(d.parent);
    }
  ...
  function zoomTo(v) {
    ...
    node
      ...
      .attr('width', function (d) {
        return 2 * d.r * k * Math.cos(Math.PI / 4);
      })
      .attr('height', function (d) {
        ...
        if (d.depth < 2) {
          ...
          jQuery('.id-' + d.id + ' div.text-box').css(
            'font-size',
            Math.round(side / 3.2)
          );
        }
  ...
  function setHeightCir(focus) {
    ...
    d3.transition()
      ...
      .attr(
        'transform',
        'translate(' + computedWidth / 2 + ',' + box.height / 2 + ')'
      );
  }
}
...
function tree(obj) {
  ...
  var diagonal = d3.svg.diagonal.radial().projection(function (d) {
    return [d.y * factor, (d.x / 180) * Math.PI];
  });
  ...
  function update(source) {
    ...
    d3.select('.legend-opt').style('stroke', function () {
      return color(source.parent.name);
    });
    ...
    nodeEnter
      ...
      .attr('class', function (d) {
        if (d.depth === 0) {
          if (
            typeof pdfLinks[slugify(d.name)] !== 'undefined' &&
            pdfLinks[slugify(d.name)].href
          ) {
    ...
    var nodeUpdate = node
      ...
      .attr('transform', function (d) {
        ...
        return d.y > 0
          ? 'rotate(' + (d.x - 90) + ')translate(' + d.y * factor + ')'
          : 'rotate(180)';
      });
    ...
    nodeUpdate
      .select('circle')
      .attr('r', function (d) {
        return d.depth ? 10 * factor : 50 * factor;
      })
      .style('fill', function (d) {
        if (d.value === 2) {
          return color2(d.parent.parent.name);
    ...
    nodeUpdate
      ...
      .attr('transform', function (d) {
        ...
        return d.x < 179.99999999999997
          ? 'translate(5,0)'
          : 'rotate(180)translate(-' + (box.width + 25) + ')';
      });
    ...
    svg.selectAll('text').on('mouseup', function (d) {
      if (!d.children) {
        for (var i = 0; i < strategies.children.length; i++) {
          for (
            var ii = 0;
            ii < strategies.children[i].origChildren.length;
            ii++
          ) {
    ...
    function setHeight() {
      ...
      d3.transition()
        ...
        .attr(
          'transform',
          'translate(' + computedWidth / 2 + ',' + (height / 2 + 10) + ')'
        );
    }
